...
for entry in tqdm(combined, desc="Parsing entries"):
    try:
        ...
        if prompt[-1] != "\n":
            prompt += "\n"
        prefix_len = len(tokenizer(prompt).input_ids)  # New X-axis value

        close_think_logprob = compute_close_think_logprob(prompt) if action == "Continue" else None
        data_by_problem[problem].append((prefix_len, action, mean_reward, close_think_logprob))
    except Exception as e:
...
for idx, (problem, entries) in enumerate(sorted(data_by_problem.items())):
    ...
    for prefix_len, action, reward, logprob in entries:
        if action == "Terminate":
            prefix_len_terminate.append(prefix_len)
            reward_terminate.append(reward)
        elif action == "Continue":
            prefix_len_continue.append(prefix_len)
            reward_continue.append(reward)
            if logprob is not None:
                logprob_prefix_len.append(prefix_len)
                logprob_values.append(logprob)

    fig, ax1 = plt.subplots()

    # Plot rewards
    if prefix_len_terminate:
        x_t, y_t = zip(*sorted(zip(prefix_len_terminate, reward_terminate)))
        ax1.plot(x_t, y_t, label="Terminate", marker="o", linestyle="-")
    if prefix_len_continue:
        x_c, y_c = zip(*sorted(zip(prefix_len_continue, reward_continue)))
        ax1.plot(x_c, y_c, label="Continue", marker="x", linestyle="--")

    ax1.set_xlabel("Prefix Length (in Tokens)")  # Updated label
    ax1.set_ylabel("Mean Reward")
    ax1.legend(loc="upper left")

    # Plot log-probs
    if logprob_prefix_len:
        ax2 = ax1.twinx()
        x_l, y_l = zip(*sorted(zip(logprob_prefix_len, logprob_values)))
