...
class GRPOTrainer(Trainer):
    ...
    def _get_patient_prompt(self, case_desc): 
    ...
    def _build_conv(self, system_prompt, prompts, responses, role1 = "user", role2 = "assistant"):
        dialogue = []
        dialogue.append({
                "role": "system",
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                outputs = self.llm.chat(all_prompts, sampling_params=self.sampling_params, use_tqdm=False)
                # outputs = self.llm.generate(all_prompts_text, sampling_params=self.sampling_params, use_tqdm=False)
                completion_ids = [out.token_ids for completions in outputs for out in completions.outputs]

                # Process in groups of num_gens for each process
                # new_outputs = []
                ...
                #     )
                #     if not has_final_diagnosis and len(group) > 0:
                #         # Choose a random completion to replace
                #         replace_idx = random.randint(0, min(num_gens, len(group)) - 1)
                        
                #         # Get the original completion text
                #         original_text = self.processing_class.decode(group[replace_idx].outputs[0].token_ids)
